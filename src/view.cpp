...
int32_t view::xoff()
{
    ...
    return Max(0, (int)(m_lastpos.x - (m_bb.x - m_aa.x + 1) / 2 + m_shift.x + pan_x));
}
...
int32_t view::interpolated_xoff()
{
    ...
    return Max(0, (int)((m_lastlastpos.x + m_lastpos.x) / 2
...
int32_t view::yoff()
{
    ...
    return Max(0, (int)(m_lastpos.y - (m_bb.y - m_aa.y + 1) / 2 - m_shift.y + pan_y));
}
...
int32_t view::interpolated_yoff()
{
    ...
    return Max(0, (int)((m_lastlastpos.y + m_lastpos.y) / 2
...
void view::update_scroll()
{
    ...
    if (m_focus->x > m_lastpos.x)
        m_lastpos.x = Max((int)m_lastpos.x, (int)(m_focus->x - no_xright));
    else if (m_focus->x < m_lastpos.x)
        m_lastpos.x = Min((int)m_lastpos.x, (int)(m_focus->x + no_xleft));

    if (m_focus->y > m_lastpos.y)
        m_lastpos.y = Max((int)m_lastpos.y, (int)(m_focus->y - no_ybottom));
    else if (m_focus->y < m_lastpos.y)
        m_lastpos.y = Min((int)m_lastpos.y, (int)(m_focus->y + no_ytop));
}
